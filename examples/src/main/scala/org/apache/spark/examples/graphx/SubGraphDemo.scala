...
object SubGraphDemo extends  App{
  ...
  //对于 users 这个 RDD 而言，其每一个元素包含一个 ID 和属性，属性是由 name 和 occupation 构成的元组
    sparkCtx.parallelize(Array((4L, ("David", 42)), (6L, ("Fran", 50)),(1,("Alice",28)),(3,("Charlie",65)),(5,("Ed",55)) ))
  ...
  //Edge case 类,边缘具有 srcId 和 dstId 对应于源和目标顶点标识符,此外,Edge 该类有一个 attr 存储边缘属性的成员
    sparkCtx.parallelize(Array(Edge(5,3,8),Edge(2,1,7), Edge(5,6,3), Edge(3,6,3)))

  val graph = Graph(usersa, relationships,false)
