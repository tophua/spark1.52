...
class PairRDDFunctionsSuite extends SparkFunSuite with SharedSparkContext {
  ...
  test("groupByKey with negative(负数) key hash codes") {    
    val pairs = sc.parallelize(Array((-1, 1), (-1, 2), (-1, 3), (2, 1)))
    val groups = pairs.groupByKey().collect()
    /* 
		结果 
  ...
  test("reduceByKey with collectAsMap") {
    val pairs = sc.parallelize(Array((1, 1), (1, 2), (1, 3), (1, 1), (2, 1)))
    val sums = pairs.reduceByKey(_ + _).collectAsMap()//返回Map类型
    assert(sums.size === 2)
    assert(sums(1) === 7)
    assert(sums(2) === 1)
  }

  test("reduceByKey with many(许多) output partitons") {
    val pairs = sc.parallelize(Array((1, 1), (1, 2), (1, 3), (1, 1), (2, 1)))
  ...
  test("join") {
    ...
    val joined = rdd1.join(rdd2).collect()//对两个key-value的RDD执行内连接
