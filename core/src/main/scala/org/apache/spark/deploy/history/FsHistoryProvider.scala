...
private[history] class FsHistoryProvider(conf: SparkConf, clock: Clock)
        ...
        .filter { entry =>
          try {
            ...
            case e: AccessControlException =>
              ...
              // every poll.
              //不要使用“logInfo”，因为这些消息可以在每次轮询上打印时都会很嘈杂
   ...
   * application. Return `None` if the application ID cannot be located.
    *重播指定日志文件中的事件,并返回有关相关应用程序的信息,如果无法找到应用程序ID，则返回“无”。
   */
  private def replay(
      eventLog: FileStatus,
    ...
    try {
      ...
      //没有应用程序ID，新的日志将在列表页面中显示不正确，因此不要列出或
      //尝试显示他们的用户界面,一些旧版本的Spark生成的日志没有应用程序ID,所以让这些版本生成的日志通过。
   ...
   * the code in previous releases.
    * 加载旧版日志目录,这假设日志目录包含单个事件日志文件(以及其他元数据文件),这是由以前版本中的代码生成的目录的情况。
   *
   ...
   * As of Spark 1.3, these files are consolidated into a single one that replaces the directory.
    * 返回指定的事件日志路径是否包含基于目录的旧事件日志,以前,应用程序的事件日志包括一个目录中的多个文件。
    * 从Spark 1.3开始,这些文件被合并到一个替换目录的文件中。
   ...
   * directory, `None` is returned, indicating that there isn't an event log at that location.
    * 返回给定事件日志的修改时间,如果状态指向一个空目录,则返回“无”,表示该位置没有事件日志
   */
   ...
   * in Spark 1.1.
    * 返回生成日志的Spark版本是否记录应用程序ID,应用程序ID已添加到Spark 1.1中。
   */
