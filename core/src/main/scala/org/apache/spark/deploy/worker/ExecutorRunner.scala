 ...
 * workerThread执行过程中主要调用fetchAndRunExecutor方法执行步骤如下
 * 1)构造ProcessBuilder
 * 2)为ProcessBuilder设置执行目录,环境变量
 * 3)启动ProessBuilder生成进程
 * 4)重定向进程的文件输出流与错误流为executorDir目录下的文件stdout与stderr
 * 5)等待获取进程的退出状态,一旦收到退出状态,则向Worker发送ExecutorStatChange消息
 * 
 */
