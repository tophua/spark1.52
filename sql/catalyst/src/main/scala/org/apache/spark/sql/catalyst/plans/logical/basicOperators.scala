...
case class Generate(
  ...
  //我们不希望将gOutput作为表达式的一部分,因为这会导致像未解析的属性等异常
 ...
 * A hint for the optimizer that we should broadcast the `child` if used in a join operator.
  * 优化器的提示,如果在连接运算符中使用,我们应该广播`child`
 */
 ...
 * A container for holding named common table expressions (CTEs) and a query plan.
  * 用于保存命名公用表表达式(CTE)和查询计划的容器
 * This operator will be removed during analysis and the relations will be substituted into child.
  * 在分析过程中将删除此运算符,并将关系替换为子项
...
case class Expand(
   ...
   * are not set for this grouping set (according to the bit mask).
    * 为子投影创建一个投影数组,如果没有为这个分组集（根据位掩码）设置那些表达式,
    * 则替换投影的表达式,这些表达式与Literal(null)等于GroupBy表达式
   */
    ...
    bitmasks.foreach { bitmask =>
      ...
      val substitution = (child.output :+ gid).map(expr => expr transformDown {
        case x: Expression if nonSelectedGroupExprSet.contains(x) =>
          ...
          //如果此组的Invalid Grouping Expression集合中的input属性将其替换为常量null
 ...
 * 具有GROUPING SETS的GROUP BY子句可以生成等效于由多个简单GROUP BY子句的UNION ALL生成的结果集。
 ...
 * Cube是GROUPING SETS的语法糖,将转换为GroupingSets,最终将转换为Analyzer中的Aggregate（..，Expand）
 ...
 * and eventually will be transformed to Aggregate(.., Expand) in Analyzer
  *
  * Rollup是GROUPING SETS的语法糖,并将转换为GroupingSets,最终将转换为Analyzer中的Aggregate（..，Expand）
 *
 * @param groupByExprs The Group By expressions candidates, take effective only if the
 *                     associated bit in the bitmask set to 1.
 * @param child        Child operator
...
case object OneRowRelation extends LeafNode {
   ...
   * of cartesian joins.
    * 计算此计划的[[Statistics]],默认实现假定输出基数是所有子计划基数的乘积,即适用于笛卡尔连接的情况
   *
